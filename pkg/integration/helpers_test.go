...
const (
	...
	skipMessage = "integration suite not configured: run `proton-cal login` and create pkg/integration/config.toml (see config.example.toml)"
)
...
func initSuite() {
	...
	if len(tc.Calendars) == 0 {
		setupSkip = "pkg/integration/config.toml lists no calendars; " + skipMessage
		return
	}
